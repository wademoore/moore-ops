...
The verified `us-east-2` account concurrency quota of 10 limits simultaneous Lambda execution across the account, but it is not endpoint-specific throttling, a monthly request limit, or a spending cap. A first deployment with reserved concurrency one was rejected because AWS requires at least 10 unreserved executions; the stack rolled back cleanly and the reservation was removed. Strict endpoint usage-plan controls would require API Gateway or another layer. The state-aware S3 cache, request-driven operation, bounded provider attempts and response limits remain the primary upstream-call and cost controls.
...
`infrastructure/sports-live-refresh/template.json` describes one arm64 Node.js Lambda, public Function URL, private versioned S3 bucket, single-key least-privilege IAM, 20-second timeout, 256 MiB memory, exact-origin environment configuration and 14-day log retention. The SAM esbuild configuration starts at `sports/lambda.js` and bundles its runtime import graph, including the explicitly pinned production `@aws-sdk/client-s3` dependency. Values are parameterized; no real bucket, account, email or secret is present.
