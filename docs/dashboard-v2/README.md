...
### Pi-hosted shadow preview

The approved long-lived preview is isolated at `/home/pi/moore-dashboard/shadow/index.html` and served by `moore-dashboard-shadow.service` on loopback-only `127.0.0.1:4174`. Its stable Pi Chromium URL is `http://127.0.0.1:4174/index.html`. Production remains on `127.0.0.1:4173` and `/home/pi/moore-dashboard/current`; the production service, timer, releases, Chromium launcher, Dashboard v1, and sports CORS are not changed.

After the one-time shadow service installation, run exactly `npm run publish:dashboard-v2:now-next` with the existing local Google auth environment and authorized Pi SSH access. The command renders current household data read-only with live sports polling disabled, validates the self-contained HTML, records production Pi invariants, uploads only the HTML to a random same-directory temporary name, revalidates its size and SHA-256 on the Pi, atomically renames it to `index.html`, verifies the loopback service and served checksum, and confirms the production invariants did not change. Diagnostics, history, credentials, tokens, source code, and household data files are never transferred.

The Windows scheduled task `MooreOps-NowNext-Shadow` has an immediate time trigger that repeats every 15 minutes plus a login trigger for resilience; `StartWhenAvailable` catches missed runs. It uses the existing external Google auth files in place and the dedicated `C:\Users\wadem\.ssh\moore_ops_pi` identity with `BatchMode` and `IdentitiesOnly`, has no visible terminal, and refuses overlapping instances. The publisher holds an exclusive lock for the complete preflight/render/upload/verify cycle. Its ignored `preview/dashboard-v2-shadow/publish.log` contains only timestamp, success/failure, selected reason code, and checksum.

Inspect: `Get-ScheduledTask -TaskName MooreOps-NowNext-Shadow | Get-ScheduledTaskInfo`. Run now: `Start-ScheduledTask -TaskName MooreOps-NowNext-Shadow`. Disable: `Disable-ScheduledTask -TaskName MooreOps-NowNext-Shadow`. Remove: `Unregister-ScheduledTask -TaskName MooreOps-NowNext-Shadow -Confirm:$false`. The task does not alter the existing production scheduler.

The render-time sports snapshot remains visible, but the shadow artifact has an empty sports-feed URL because the production endpoint intentionally permits only the production origin. A shadow-only meta refresh reloads the complete document every five minutes so an open Pi Chromium kiosk adopts atomic publishes without user action. Do not broaden production CORS for the preview.

Rollback/removal affects only the shadow: stop and disable `moore-dashboard-shadow.service`, remove its unit, run `systemctl daemon-reload`, and remove `/home/pi/moore-dashboard/shadow`. Do not touch `moore-dashboard.service`, `moore-dashboard-refresh.timer`, `/home/pi/moore-dashboard/current`, or the production Chromium launcher. The temporary Windows token viewer was retired after the first Pi-hosted publish passed independent service, binding, checksum, HTTP, and production-invariant verification.
