...
- **VPSU name discrepancy.** Swimmer names in VPSU's Top-50 Times CSV/rankings data can differ from the same swimmer's name in the team's own meet-entry system (`league-results-v2.json`). Confirmed case (2026 season): "Ryland Fidler" (WT, Boys 7-8) is recorded as `"Fidler, John"` in VPSU's system — same swimmer, same team, times match exactly across all corroborated entries (07/13 Boys 8&Under 25m Fly 30.07S; 07/20 Boys 7-8 25m Free 21.75S; 07/20 Boys 7-8 25m Back 27.91S). "Ryland" is the name he goes by; "John" is simply how VPSU's system recorded him. When cross-referencing WT swimmers against `vpsu-rankings.json` league-key data, a first-name mismatch relative to `league-results-v2.json` is not conclusive evidence of two different swimmers — check whether team, age group, event, date, and time all align before concluding a name is unmatched. Do not silently correct ingested VPSU data to the team-preferred name: the `league` key in `vpsu-rankings.json` should faithfully mirror what VPSU published; note discrepancies in editorial findings rather than editing the source data.
