...
Notes: Relay times expressed as MM:SS.xx. Place = overallPlace from relay-results-v2.json (native from PDF Pl column). Field = count of legal finishers as printed in the source PDF's Pl column; NS (no-show) entries and at least one known DQ-formatting variant are silently absent from relay-results-v2.json rather than deliberately excluded by rule — see CLAUDE.md Known Open Items for the underlying parser limitation. No relay records apply for these age group / event combinations. "Team A" / "Team B" labels are assigned by time; the source data does not label relay entries with team designators.
