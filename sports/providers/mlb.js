...
export function normalizeMlbGame(game, feed, record = null) {
  ...
  return { id: String(game.gamePk), feedId: feed.id, organization: feed.organization, sport: 'baseball', league: 'mlb', startTime: new Date(game.gameDate).toISOString(),
    ...
    result: state === 'final' ? (teamScore > opponentScore ? 'W' : teamScore < opponentScore ? 'L' : 'T') : null, record, rank: null };
}
export async function fetchMlbFeed(feed, { now = new Date(), fetchImpl = fetch, timeoutMs } = {}) {
  ...
  const standingsUrl = `${BASE}/standings?leagueId=104&season=${season}&standingsTypes=regularSeason`;
  ...
  let record = null;
  for (const league of standings?.records || []) for (const row of league.teamRecords || []) if (String(row.team?.id) === String(feed.teamId)) record = `${row.wins}-${row.losses}`;
  const events = schedule.dates.flatMap(date => date.games || []).map(game => normalizeMlbGame(game, feed, record)).sort((a, b) => +new Date(a.startTime) - +new Date(b.startTime) || a.id.localeCompare(b.id));
  return { id: feed.id, organization: feed.organization, sport: 'baseball', provider: 'mlb', fetchedAt: new Date().toISOString(), events };
}
