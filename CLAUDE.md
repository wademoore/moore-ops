...
- **Relay parser silent-drop bugs fixed (Phase 1, commit 409d2fe, July 2026) — Phase 2 re-parse pending** — two silent-drop bugs in `scripts/pdf-reload-parser.mjs` were fixed: (1) NS/DNF/SCR tokens in relay official column were rejected by the time/DQ regex; (2) the 1-tab SA fallback path read only parts[1] for times, dropping rows where seed is in parts[1] and official is in parts[2]. Both fixes shipped in commit 409d2fe with 8 regression tests added, 635 passing. **Phase 2 not yet done:** scope is 54 non-SA 2026 meets + 1 Summer Awards 2026 meet + 208 history meets (2022–2025) = 263 meets total. All 263 need re-parsing with the fixed parser to recover the ~43 dropped rows (39 NS + 3 DNF rows across 22 history meets, plus 2 non-SA 2026 NS rows + 1 KW B DQ row + 7 SA NS rows) and to backfill overallPlace/overallCount for the 455 non-SA relay-results-v2.json rows and all 2034 relay-results-history-v2.json rows (those fields are absent, not null, for rows parsed before commit 229cc4f). Phase 2 is a separate future session — do NOT re-parse PDFs or touch data/ JSON files until that session is explicitly opened.