...
- **Relay parser silent-drop bugs fixed (Phase 1, July 2026) — Phase 2 re-parse pending** — two silent-drop bugs in `scripts/pdf-reload-parser.mjs` were fixed: (1) NS/DNF/SCR tokens in relay official column were rejected by the time/DQ regex; (2) the 1-tab SA fallback path read only parts[1] for times, dropping rows where seed is in parts[1] and official is in parts[2]. Both fixes are in production code (commit pending) with 8 regression tests added, 635 passing. **Phase 2 not yet done:** all 259 non-SA 2026 meets + 205 history meets need to be re-parsed with the fixed parser to recover the ~43 dropped rows (42 NS/DNF rows + 1 KW DQ row confirmed by audit) and to backfill overallPlace/overallCount for the 455 non-SA relay-results-v2.json rows and all 2034 relay-results-history-v2.json rows (those fields are absent, not null, for rows parsed before commit 229cc4f). Phase 2 is a separate future session — do NOT re-parse PDFs or touch data/ JSON files until that session is explicitly opened.