...
| `data/league-results.json` (v1) | 6,772 | 2026 WT meets only; untouched |
...
| `waves-record-progression` | **committed** `.claude/skills/waves-record-progression/check.js` | Manual — WT all-time record progression history; reads `league-results-history-v2.json`, `relay-results-history-v2.json`, `league-results-v2.json`, `relay-results-v2.json`; WT-only filter; console-only output; no test coverage. **Note:** v2 relay files use `"Boys/Girls 9-18"` ageGroup labels, while `waves-team-records.json` uses `"Men/Women Open"` — relay record progressions (3 records) are unreconstructable. The same `RELAY_AGEGRP_MAP` normalization applied to `waves-team-record-check` (July 2026) could fix this, but has not yet been applied to this script. |
...
- **Champs/Summer Awards history data loaded into non-v2 files — consumer skills can't read it yet (July 2026, commits `e16edc7` / `9c833d8` / `bb3afaf`):** The following meets are now in `league-results-history.json` and `relay-results-history.json` (non-v2): 2024 VPSU Champs (individual + relay), 2025 VPSU Champs (individual + relay), 2026 Summer Awards (individual + relay). The following are permanently unrecoverable — no source PDF exists: 2022 Champs individual results, 2022 Summer Awards (any), 2023 Champs (any), 2023 Summer Awards (any), 2024 Summer Awards individual results. **Still open:** `waves-champs-qualifier/check.js` reads `league-results-history-v2.json`, and `waves-record-progression/check.js` reads all four v2 history/current files — neither reads the non-v2 history files where Champs/SA data now lives. Near-miss resolution (who has a historical Champs result close to a team record?) and qualifying-swim reachability checks cannot use this data until either (a) the non-v2 files are repointed to v2 files in those scripts, or (b) the Champs/SA data is separately parsed into the v2 history files (`league-results-history-v2.json` / `relay-results-history-v2.json`). This is the next real decision point for this data.
