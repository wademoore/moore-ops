...
**Note:** the four committed-script skills (`waves-champs-qualifier`, `waves-team-record-check`, `waves-div1-simulation`, `waves-div1-2027-projection`) run via `node <path>/check.js` (or `project.js`) and must not be re-derived manually from their SKILL.md — the script is authoritative. Prose-only skills are re-derived fresh from SKILL.md each invocation.
...
**✓ 594 unit tests passing, 0 failing (current baseline as of July 2026 — 430 post ageGroup fix, +12 from fractional-points / 1-tab relay / DQ-handling fixes, +10 from FIX 1 Unicode names / FIX 2 multi-line wrap / FIX 3 relay NT, +14 from HIST EXT null-byte / EXH / non-scoring-finisher / SCR + pre-existing double-quote name fix, +1 from HIST EXT 6 X-prefix name-wrap, +2 from HIST EXT 7 NT-official EXH rows, +8 from waves-div1-simulation nearest-meet rewrite (17 new / 9 removed), +2 from HIST EXT 8 parenthetical-nickname EXH rows, +85 from waves-div1-2027-projection skill commit 868c84c)**
...
- **2022 COMPLETE; 2023 COMPLETE; 2024 COMPLETE; 2025 not yet run** — Post-Batch-6 state: `league-results-history-v2.json`: 60,283 rows (2022: 21,250 | 2023: 18,041 | 2024: 20,440 | 2025 trial: 552). `relay-results-history-v2.json`: 1,508 rows (2022: 509 | 2023: 458 | 2024: 541). 2022 skipped: 2022-06-13-glt-at-gs (no PDF). Data gaps: all confirmed genuine small-roster Div3 meets. kw-at-ql 2023-06-26: 0 relay rows — PDF-verified (no relay content in 602 lines). **New 2024 parser gaps (not in 2022/2023):** (1) Swimmer "Kun 3rd, Kube" (Village Green, age 10): last name contains "3rd" ordinal suffix with digit; name regex rejects the row — 12 dropped rows across 5 VG meets (vg-at-kw, vw-at-vg, sh-at-vg, ip-at-vg, vg-at-glt). Fix: extend name character class to allow digits in specific ordinal-suffix context. (2) Relay tie format "2*" (kw-at-ftc 07-22): tied relay results use `2*` prefix — 2 dropped relay rows. Fix: extend relay place regex to handle `\d+\*`. Both gaps are below the >5 genuine-warning threshold; all 53 2024 meets parsed and committed. **Planner inventory warning:** the 2023 Planner spec incorrectly claimed "all 48 filenames use the 'at' pattern, no vs. ambiguities" — 3 "vs." files were found and resolved during manifest-building using 2022 precedent. Do not trust Planner PDF-inventory counts or naming-pattern claims for 2024/2025 at face value; verify directly from the directory listing.
