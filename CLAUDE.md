...
- **A within-club finish goes in `place`, never `overallPlace`.** `derivePlacementString()` in `swimParser.js` builds its placement string from `overallPlace`/`overallCount` and ignores `place` entirely, so `place` records the finish faithfully while keeping it off every rendered surface. Verified: the four `opheliaPBRows` that `events757` produces all report `placement: null`. (The count is of *configured PB rows*, not of SCY rows in the file, of which there are 20.) There is no `lane` field anywhere in `swim-results.json`, and no seed or entry-time field at all, so lane, entry time, round, event number and start clock have no home and are not stored. `heat`/`heatPlace` do have homes; `totalHeats`/`totalSwimmers` are written `null` because the source did not give them. **Two heat conventions coexist in this file** — 28 rows use `heat`/`totalHeats`/`heatPlace` alongside `place`/`totalSwimmers`, and 6 (the 2026-06-15 batch) use `heatNumber`/`heatCount` alongside `overallPlace`/`overallCount`. The KickOff rows take the first, which is both the majority and the one the DQ convention above names.
  ⚠ **`place` does not mean the same thing on either side of this change, and it is not clear which meaning is right.** In `data/swim-757-results.json` the parser's `place` equals `totalSwimmers` on **17 of 17** non-DQ rows, and **14 of the 17 pre-existing SCY rows** correspond to one of those and carry the identical `place` — so for those 14 the value is either a field size mislabelled as a finish, or a genuine last place every single time, and the source cannot settle it from here. **The other 3 — the October Spooktacular rows, `place` 32, 31 and 17 — have no counterpart in either parser file, so for them an ordinary mid-field finish is a live third possibility and the disjunction above does not apply.** (Those SCY rows that do have a counterpart match on event and time as well as on `place`: the 50-yard ones among them failed a time match until the +20.00 defect recorded below was corrected.) The KickOff rows put real finishes (8, 59, 11) in that column. Harmless today because **no consumer anywhere reads `place` off a swim row**, but do not aggregate or compare that column across the two eras without resolving this first.
...
### Milestones resolve from OUR fixtures only (Sept 16, 2026)

`selectSeasonMilestone()` narrows `games[]` to rows our team plays in **before** taking the
earliest date and **before** judging ambiguity. Compared by the league's numeric team id (or
the legacy string abbr), never by mascot. A season declaring neither id nor abbr is left
unrestricted, which keeps a team-less season file resolving as it did — and avoids a subtler
failure: `myKey` would otherwise be `null`, and a practice row carries `away: null`, so a
null-keyed comparison would match the practice **on its empty side**, selecting a row for the
wrong reason rather than failing closed.

**This was forced by loading the division schedule, and the suite is what caught it.** Four
...
**This is the discipline `flagFootballIdentity.js` already had, arriving late in its sibling.**
`findFixtureByDate()` was written with exactly this narrowing and a comment explaining it,
against a case that was then unreachable. `selectSeasonMilestone()` was not. One file's foresight
does not protect another's, and "unreachable in current data" is a statement about the data, not
about the code.

⚠ **It changed a documented, tested invariant, and that is recorded rather than smoothed over.**
`selectSeasonMilestone()` now READS `home`/`away`. `test/flagFootballParser.test.js`'s *reads
only immutable fixture columns* mutated them alongside the scores to prove they were not read;
that mutation set now excludes them, and the test asserts instead that they stay unreachable
**from the returned projection**. The two claims are different and only the second was ever the
one worth keeping: `home`/`away` are not result-bearing (which two teams are scheduled does not
change when the game is played), so reading them costs nothing against the "resolves identically
mid-event" rule, and `sportsFixture` likewise reads teams while refusing `played` and the scores.
What keeps them out of `MILESTONE_FIXTURE_FIELDS` is that home/away is **nominal** in this
league — every fixture is at the same complex — and must never reach a renderer as a travel cue.
Reading a column to find our row and exposing it to a surface are different questions.

**The narrowing is asserted, not assumed.** `narrows to our own fixtures before judging
...
**seven** flag football occurrences and `fall-2026` holds **six rows Myles plays
in** (of 21 rows in total since the division schedule was loaded on Sept 16,
2026 — the other 15 are fixtures between teams he does not play in, and
`findFixtureByDate()` narrows them away before judging ambiguity, which is
exactly the case the comment quoted below anticipated). The seventh occurrence,
`2026-10-25` "Flag Football: Week 6 — Practice + Game / Playoffs (Yorktown)",
still has no season row. ⚠ **The reason changed on Sept 16 and the conclusion did
not.** It used to be that Oct 18 was the last week the league had posted; the
Oct 25 postseason *is* now posted — five slots: two consolations, two semifinals
and a championship — but every participant is TBD, and this file has no way to
express a scheduled slot with unknown participants without inventing teams. So
the row is still absent, now deliberately rather than for want of a source. The gap sweep's reach is the calendar pull's own reach — `getCalendarEvents()`
...
on a crowd even though exactly one row is ours. ⚠ **Both were described here as
unreachable until Sept 16, 2026, and both are now live.** `fall-2026.games`
carries the whole published division schedule — 20 regular-season fixtures, four
on every fixture Sunday — so the second case is exercised on every one of Myles's
game dates, and the narrowing is what makes them resolve instead of failing
...
|---|---|---|---|---|---|
| `npm test` with `DASHBOARD_BROWSER_PATH` set | 2745 | **2745** | **0** | **0** | 62721.017631 ms |

Measured on `claude/hopeful-dirac-nsnbp1`, branched from `origin/main` at **`afe1876`**.
`git fetch origin main` was run before deriving the base, per the standing warning, and it
mattered: the ref was stale at `13ad7f5` and the fetch moved it to `afe1876`. `HEAD ==
merge-base == origin/main`.

**The base was re-measured in this session, after `npm ci` and before any edit: 2744 / 2744 /
0 / 0, 64665.062329 ms.** That is NOT the 2704 the superseded entry records — `main` has taken
#102 and #103 since. Re-measure; the run costs less than the correction.

This change adds **+1**, in one existing file:

| File | before | after | delta |
|---|---|---|---|
| `test/flagFootballParser.test.js` | 48 | 49 | +1 |
...
- **One was a genuine invariant change**, `reads only immutable fixture columns`. It mutated
  `home`/`away` alongside the scores to prove they were not read. They are read now — see
...
### Superseded baseline — measured Sept 15, 2026 on PR #89's 757 card follow-up
...
### Superseded baseline — measured Sept 15, 2026 on the latest-757-meet-view branch
...
- **✓ RESOLVED Sept 17, 2026 — Ophelia's 50-yard times in `swim-results.json` were recorded
  exactly 20.00s slow, and `pb-records.json` carried one of them (found Sept 12, 2026;
  corrected against the raw Hy-Tek meet files Sept 17, 2026).** Found while recording the
  757swim Season KickOff. Of the rows the two files share, **all 14 sub-minute times agreed
  exactly** and only the minute-format ones diverged — each by precisely +20.00. *(An earlier draft said "every sub-minute time in the file agrees",
  which overstates it: the three October Spooktacular rows — 32.67, 35.74 and 41.09 — have no
  counterpart in either parser file, so they agree with nothing. A Reviewer round caught it.)*

  | Row | `swim-results.json` (as recorded, before the correction) | `swim-757-results.json` | Δ |
  |---|---|---|---|
  ...
  **A second source agrees against the hand-maintained file — but it is not independent of the
  first, and that cuts against the evidence rather than for it.** The KickOff's own Meet Mobile
  entry time for the 50 Back was **1:09.23** — identical to the parser's value and 20.00s off
  what was recorded at the time. USA Swimming seeds from the official best time, so both the seed and the
  parser plausibly descend from the same club Hy-Tek/SWIMS records; agreement to the hundredth
  makes a common origin *more* likely, not less. What they jointly established is that
  `swim-results.json` disagreed with the club's own records, which was enough to justify the
  open item and not enough to call it two independent confirmations. In all three the minutes digit is 1 and the tens-of-seconds
  ...
  **The same meet's *other* entry time is the sharper evidence, and it was missed on the first
  pass.** The KickOff seeded her 25y Breast at **41.09**, which equals the recorded PB to the
  hundredth. So the seeding data agrees where the time is sub-minute and disagrees by 20.00
  where it is not — which points at a minutes-transcription defect rather than a general
  disagreement between the sources. ⚠ **From the seeds alone that is n=1 on each side** (41.09
  and 1:09.23; the Fly was NT), so the seeds *suggest* the split rather than establish it. What
  carries it is the parser overlap, where the same split holds 14 sub-minute agreements to 3
  minute-format divergences.
  One caveat on the corroboration, since resolved: the two files also disagreed on that swim's
  **date**, and the `.cl2` per-event date settled it at 2026-01-11 — the Splash and Dash export
  runs Jan 9–11 and puts that swim on the last of them. The date was corrected with the time.

  **Deliberately not fixed at the time.** It was three pre-existing rows plus
  ...
  reads is Wade's call. It was taken on Sept 16, 2026 — see the Updater skill's source-precedence
  decision of that date — and the rows were corrected against the raw Hy-Tek files rather than
  against that parser output. `pb-records.json`'s 50y Backstroke entry had already been
  overwritten by the 69.08 KickOff swim, which is her fastest under either reading, so that key
  needed no change; the stored 50y Freestyle personal best did, and was corrected.

  **What it cost that change:** the 50y Backstroke note stated no improvement figure at all,
  because "20.15s" would have been built on the disputed value. Nothing renders it —
  `sports-config.json`'s `events757` has no `50y Back`, so that row reaches no surface. The note
  now names the prior swim the meet file records, still without an improvement figure. The
  Freestyle rows and the Backstroke row were corrected together, in one pass.

- **The Waves/757 league separation is a single character in a string, and the Sept 12, 2026
  KickOff PBs made it consequential (found by a Reviewer round; pre-existing, not fixed).**
  `.claude/skills/waves-champs-qualifier/check.js` iterates **every** `swim-results.json` row
  for the Moore kids with **no course, no league and no `dq` filter**. The only thing keeping a
  757 yards row out of a Waves Champs qualifier post is that all 52 keys in `helpers.js`'s
  `standards` are metric, so `standards['Girls 8&Under|25y Breaststroke']` is `undefined` and
  `tryQualify`/`tryNearMiss` both return before recording anything. Verified empirically: the
  skill's output is **byte-identical** before and after these three rows were added.

  What changed is the cost of that separation failing. Both new PBs are **faster than the metric
  Champs standard for her bracket** — 33.37 against a `25m Breaststroke` standard of 34.00, and
  34.44 against a `25m Butterfly` standard of 37.00. So any future yard standard, or any
  normalization that folds `25y`→`25m`, silently promotes an unofficial intrasquad swim into a
  Champs qualification in a Facebook draft. The sibling skill already shows the structural fix:
  `waves-team-record-check/check.js` filters `r.course === 'SCM'` outright. Not done here —
  it is a skill file, outside an Updater data change, and it deserves its own scoping pass.
...
- **✓ (1) RESOLVED Sept 16, 2026 — the full division schedule was published and loaded.
  (2) the tie column is still produced and displayed nowhere.** Both were raised by a round-2
  Reviewer pass on Sept 10 as consequences of the season that had just been added. Recorded
  because each becomes *visible output* on Sept 20, the first game day.
  ...
  **(2) is unchanged and still open.** `parseFlagFootball` emits a `t` (ties) field on every
  ...
  frozen v1 heads its `Team | W | L | PF | PA`, and the email renders no standings at all — so
  a team at `3-0-1` will show `seasonRecord: "3-0-1"` above a standings row reading `3 / 0`:
  four games, three shown. Flag football ending level is the stated premise of the tie fix, so
  this is reachable rather than hypothetical. It is a presentation decision on a surface Codex
  owns, which is why it is still not fixed here; the option is a tie column.

  ⚠ **Closing (1) made (2) worse, and that is worth stating rather than leaving implicit.**
  While the table held only our own results, a hidden tie column cost one row its fourth game.
  A real 8-team division table with 20 fixtures can now disagree with itself in eight rows at
  once, under a heading a viewer reads as authoritative.
...
- **757swim full-field parser integration into `swimParser.js`/`builder.js` (future task)**: `scripts/parse-757swim-full.mjs` is built and Reviewer-approved. Integration target: `data/league-results-757.json` (21,491 individual rows) and `data/relay-results-757.json` (668 relay rows). The deprecated `data/swim-757-results.json` and `data/swim-757-relays.json` (Ophelia-only, from the deprecated parser) remain in `data/` until integration is complete — per spec §5, they are retained for use by `swimParser.js` during the transition.
  ...
  ⚠ **The last sentence of this item used to read "Nothing in `digest/` or any skill reads from either the old or new 757swim output files yet." That stopped being true on 2026-09-16.** `digest/builder.js` now reads `data/league-results-757.json` and passes it down through `athleticsParser.js` → `swimParser.js` → `latest757Meet.js` to `digest/priorBest.js`, which uses it as covered history for the prior-best fields on `athletics.opheliaLatest757Meet`. The file is packaged in `dashboard-artifact/package-inputs.json`. **That is not the integration this item describes** and does not close it: the full-field rows are read only as comparison history for one swimmer's 757 races. No *digest module or skill* reads the relay file, the canonical-id crosswalk, or the full roster — but `scripts/canonicalize-757-swimmers.mjs` reads `data/relay-results-757.json` and both reads and writes `data/swimmers-757.json`, which is the append-only ID registry, so neither file is unused and neither is safe to delete: losing the crosswalk restarts canonical IDs at 1 and destroys the re-run stability that script's spec requires. The *deprecated* pair is still read by nothing.

  The retention clause above is now also worth re-reading before acting on it. `swimParser.js` does not read `swim-757-results.json` today and the prior-best work deliberately did not add it — `digest/priorBest.js` records why it chose the current parser output over the deprecated one. So "retained for use by `swimParser.js` during the transition" describes a plan, not a live dependency.
