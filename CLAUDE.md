...
- **NS (no-show) relay rows silently dropped by `scripts/pdf-reload-parser.mjs`** — the literal "NS" token in the official column of a relay row does not match the parser's time/DQ regex, so no-show relay entries produce no output row and no warning. The entry is completely absent from `relay-results-v2.json`; a downstream consumer cannot distinguish "no NS entries in this event" from "NS entries were dropped." Confirmed active in the 2026-07-25 Summer Awards PDF: Girls 9-10 (WPD A and WPD B NS), Boys 11-12 (WPD A, WPD B, FDC A NS), Girls 11-12 (FDC A, QL A NS) — 7 dropped NS entries in this single meet. Likely affects relay NS entries across all 54 2026 meets and all history-reload meets parsed to date. No fix scoped; the underlying regex would need to handle "NS" as a known official token for relay rows (returning a dropped/placeholder row with `dq: true, time: null` or similar) rather than silently producing no output.
- **3-tab-formatted DQ relay rows silently dropped by `scripts/pdf-reload-parser.mjs`** — for relay DQ rows where the source PDF places the "DQ" token at `parts[2]` (tab-split index 2) rather than `parts[1]`, the parser's fallback check only inspects `parts[1]` and misses the row entirely. No output row is written; no warning is emitted. Confirmed: Kingswood Klams B, Girls 9-10, 2026-07-25 Summer Awards — entirely absent from `relay-results-v2.json` (event 34 has exactly 10 rows, zero DQ rows for this ageGroup/event, despite the source PDF explicitly printing a KW B DQ entry). Scope of impact across other meets is unknown — no systematic audit of all parsed PDFs has been done. No fix scoped; the parser's relay DQ fallback needs to check additional tab-split positions or use a regex that spans the full row.