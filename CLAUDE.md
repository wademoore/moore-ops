...
### Source files (`data/sources/757/` — intake only, not read by any live code)

Raw Hy-Tek MeetManager exports for 757swim (USA Swimming) meets Ophelia attended. **757swim is Ophelia only — Myles does not participate in 757swim in any capacity.** One folder per meet at `data/sources/757/<YYYY-MM-DD>-<slug>/`, each containing:
- The results PDF as downloaded from SwimTopia/the meet host (where available; a small number of meets have no PDF)
- `.cl2` and `.hy3` files extracted from the meet's Hy-Tek export zip — original Hy-Tek filenames preserved, zip wrappers discarded

15 meets committed covering the 2025–26 short course season (commits `0035f24` and `eb4bf29`): 9 757-hosted meets (Battle of the Burg through Spring Challenge) and 6 attended at other clubs (TIDE, BASS, NOVA ×2, SRVA/EZ Super Sectionals, VA LC Senior Champs). No parser has been built — this directory is intake-only. Nothing in `digest/` or any skill reads from `data/sources/` yet.
...
- **757swim is Ophelia only.** Myles does not participate in 757swim (USA Swimming) in any capacity. SCY rows (`course: "SCY"`) in `swim-results.json` are Ophelia's results exclusively. Any code or skill that filters `swim-results.json` for 757swim/SCY data should not expect Moore Myles rows there.
...
- **757swim source-file intake complete for 2025-26 SC season (July 2026, commits `0035f24` / `eb4bf29`):** 15 meet folders added to `data/sources/757/` — 9 757-hosted meets (Battle of the Burg Sep 19, IMX/IMR Kickoff Oct 10, Fall Fiesta Oct 25, Grand Illumination Dec 5, NYE Distance Time Trial Dec 31, Splash and Dash Jan 9, SE 8&U District Champs Feb 7, SC Send-Off Mar 20, Spring Challenge Apr 25) and 6 attended elsewhere (TIDE Spring Shockwave, BASS Jim Frye Memorial, NOVA LC Senior Classic, SRVA/EZ Super Sectionals, NOVA Spring Splash, VA LC Senior Champs Jul 9–12). Each folder contains the results PDF (where available) plus the Hy-Tek `.cl2`/`.hy3` export pair. Ophelia only — Myles does not swim 757. No parser has been built; `data/sources/757/` is intake-only and is not read by any live code.
...
- **2024/2025 VPSU Champs and 2026 Summer Awards loaded into history files (July 2026, commits `e16edc7` / `9c833d8`):** `scripts/archive/parse-champs-history.mjs` (retired one-time script; now in scripts/archive/) parsed the Champs and Summer Awards PDFs into `league-results-history.json` (9730 → 13574 rows, +3844) and `relay-results-history.json` (114 → 286 rows, +172). Each new row carries `meetType` (`"Champs"` or `"Summer Awards"`); Summer Awards rows where the swimmer achieved a CHMP qualifying standard carry `qualifyingSwim: true` (66 rows total). Existing dual-meet rows have no `meetType` field (no backfill). Open-class (18&Under) Champs relays are labeled `"Men Open"` / `"Women Open"` to match the dual-meet convention; age-specific Summer Awards relay brackets (8&Under, 9-10, 11-12, 13-18) keep their natural labels.

  **Parser extensions added for 2025 Champs PDF format** (Meet Maestro "Achv" column with VC suffix): (1) `isSkipLine` now discards year+time championship-record lines (e.g. `2004 1:08.95`); (2) `parseIndividualRow` main regex accepts `VC` suffix alongside `CHMP`; (3) `DATA_ONLY_LINE` and `FULL_RESULT_END` constants updated to allow VC in wrapped-name continuation data lines. 10 new unit tests cover these cases (VPSU CHAMPS EXT); test count 635 → 645.

  **Unrecoverable meet data gaps — not in history files and not recoverable without PDFs:**
  - **2022 VPSU Champs individual** — only a Team Scores PDF is available; no individual results PDF.
  - **2022 Summer Awards** — no PDF specified; out of scope for this load.
  - **2023 VPSU Champs** — no PDF specified; out of scope for this load.
  - **2023 Summer Awards** — no PDF specified; out of scope for this load.
  - **2024 Summer Awards individual** — only a Team Scores PDF is available; no individual results PDF.
...
- **v1→v2 cutover for `swimParser.js` complete (July 2026, commit 9fad597, Steps 1–6):** `digest/builder.js` now reads `league-results-v2.json` and `swim-annotations.json` on startup and passes them through `athleticsParser.js` → `swimParser.js`. For Moore Waves (SCM) results, `swimParser.js` builds a v2 match-set (all Moore Myles / Moore Ophelia rows) and uses it to filter `swim-results.json`, retaining only rows with no v2 equivalent (prior seasons, SCY/757 results). V2-sourced rows carry proper `overallPlace`/`overallCount` (real placement strings) and overlay `pb`/`note` from `swim-annotations.json`. `derivePlacementString` simplified: heat clause removed (v2 rows carry no heat fields). Dry-run confirmed: all 7 active event placements flipped from `null` to real values; all other fields (seconds, pb, isNewPB, isFreshPb, delta, previousPbSeconds, seasonBestSeconds) identical to baseline; 3 cosmetic meet-name differences (`"WT vs EH"` → `"EH vs WT"`, `"WT vs Powhatan Secondary"` → `"PS vs WT"` — PDF-sourced naming convention) accepted by Wade. Live verification: dashboard rendered 18.9KB, uploaded to Drive at 7:56 AM ET, email delivered to both addresses, no errors.
...
- **Champs/SA data now in v2 files; legacy non-v2 files archived (July 2026):** The 2024/2025 VPSU Champs and 2026 Summer Awards rows (3,844 individual + 172 relay) were loaded into the non-v2 history files by commit 9c833d8, then migrated into the v2 history files in commits 032b078 and 6b3b7f9. The legacy non-v2 history files have since been archived to `data/archive/`. Consumer skills (`waves-champs-qualifier`, `waves-record-progression`) now read from v2 and have access to the Champs/SA data. Permanently unrecoverable meets (no source PDF): 2022 Champs individual, 2022 Summer Awards (any), 2023 Champs (any), 2023 Summer Awards (any), 2024 Summer Awards individual.
- **`scripts/archive/parse-champs-history.mjs` has no dedup-on-rerun logic — future Champs/SA years require a new approach (July 2026):** The retired script appended rows to the non-v2 legacy files without checking for pre-existing rows from the same meet. If it were rerun, it would accumulate duplicates rather than replacing prior output. The script is now archived alongside the legacy files it targeted. **Any future Champs/SA year** (e.g. 2027 Champs) should use one of these approaches: (a) write a new script targeting `league-results-history-v2.json` and `relay-results-history-v2.json` directly, with slug-based dedup matching `pdf-reload-parser.mjs`'s `clearRowsForSlug` pattern; or (b) parse with the retired script's logic into a staging area and perform the migration manually as was done in July 2026 (commits 032b078 + 6b3b7f9). Either way, the 2024 Champs individual `sourcePdf: null` gap (AM+PM sessions combined; per-row attribution unrecoverable without re-parsing) would also need resolution if provenance matters for the new year's rows.
...
- **Relay parser Phase 2 complete (commit 4f061af, July 2026) — Phase 3 complete (July 2026)** — Phase 1 (commit 409d2fe) fixed two silent-drop bugs in `scripts/pdf-reload-parser.mjs`; Phase 2 re-parsed all 263 meets (55 x 2026, 208 x 2022–2025) with the fixed parser, rebuilding relay result files from scratch (individual result files untouched). Final row counts: `relay-results-v2.json` 519 → 577 rows (+58); `relay-results-history-v2.json` 2034 → 2071 rows (+37). Key recoveries: Summer Awards 2026 +8 rows (7 NS + 1 KW-B DQ, exactly as audited); history +37 rows (NS/DNF drops across 2022–2025). All non-DQ rows in both files now have `overallPlace` (integer for placed rows; null for B/C relay entries — secondary relay squads that compete in the same event but are not part of the officially-ranked A-relay standings, and thus carry no placement; `exhibitionRelay` is absent on these rows (they are not exhibition entries in the field's intended sense)) and `overallCount` (integer, 0 rows missing this field). Before Phase 2, 455 non-SA 2026 rows and all 2034 history rows had both fields completely absent. 0 parse errors across all 263 meets. **Phase 3 complete (July 2026):** 14 source-PDF spot-checks passed — 9 DQ/NS/DNF rows spanning 2022–2026 regular season and Summer Awards (types: DQ ✓, NS ✓, DNF ✓) plus 5 B/C relay rows (EXH label in PDF, `overallPlace: null` in JSON, times verified to the second). `waves-team-record-check`: 9 records unchanged, EH vs WT Women Open 200m Freestyle Relay near-miss (+0.29s) still surfacing. `waves-record-progression`: relay "no results found" is pre-existing (RELAY_AGEGRP_MAP not applied to progression script — separate open item above). Dashboard pipeline has zero double-counting risk (relay files not read by digest builder). npm test 635/635 passing. Post-dedup final row count: `relay-results-v2.json` 575 (−2 WPD NS dedup); `relay-results-history-v2.json` 2071 unchanged. Known limitations: (1) WPD NS dedup at SA events #34 and #65 — two distinct NS entries per event in PDF, one JSON row each because swimmer lists are null on both (correct dedup outcome, not a data error); (2) QL Girls 11-12 SA event #66 swimmer list captures 3 of 4 names (Dafashy dropped by relay-swimmer nickname/comma parsing — relay result itself correct, non-blocking). Should-fix items (not blocking): `waves-team-record-check`, `waves-div1-simulation`, and `waves-div1-2027-projection` include B/C relay rows without `overallPlace != null` filter; no false positives in current data (B relay times are slower than A relay times in practice). Relay swimmer-name extractor drops names in Dafashy-style nickname/comma format — see separate open items below.
