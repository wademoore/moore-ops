...
- **✓ RESOLVED Sept 17, 2026 — Ophelia's 50-yard times in `swim-results.json` were recorded
  ...
  757swim Season KickOff. Of the rows the two files share, **all 14 sub-minute times agree
  ...
  **A second source agrees against the hand-maintained file — but it is not independent of the
  ...
  the recorded one. USA Swimming seeds from the official best time, so both the seed and the
  parser plausibly descend from the same club Hy-Tek/SWIMS records; agreement to the hundredth
  makes a common origin *more* likely, not less. What they jointly establish is that
  `swim-results.json` disagrees with the club's own records, which is enough to justify the
