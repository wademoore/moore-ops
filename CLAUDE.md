...
- **Next Two Weeks is complete, not prioritized (Aug 29, 2026):** Dashboard v2 renders every eligible event in the 14-day window after the established menu/Centers exclusions and consecutive-repeat collapsing. It must not impose an event-count capacity, priority-rank days, or replace hidden calendar entries with a `+N more` indicator. Priority selection belongs in NOW/NEXT and On the Horizon; Next Two Weeks is the complete schedule view. Dense schedules adapt with the `upcoming-dense` presentation so all rows remain above Athletics on the 2560×1440 television canvas.
