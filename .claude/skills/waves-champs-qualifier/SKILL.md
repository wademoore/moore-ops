...
Block 2 prints `✨ FIRST TIME EVER` under any new-this-week entry where the swimmer has no prior qualifying swim in **any** event at any point strictly before the swim being evaluated. Implemented in `hasAnyPriorQual()` (`helpers.js`). The build-out history lives in git (CLAUDE.md's "first time ever" changelog entry, removed when the changelog was); the caveats that govern *running* this skill are below, carried over from it (figures that entry later superseded are dropped).

**Scope caveat:** The tag only fires for entries gated as new-this-week (`earliestQualDate.get(qkey) >= WEEK_DATE`). It does not retroactively tag someone whose first-ever qualification happened earlier this season and is no longer "new this week" by the time you run it. A one-time backfill script (not committed; lives in the Claude session scratchpad) exists for producing a full season-to-date first-time-ever list — re-run it fresh on request rather than assuming a prior run is current.
...
**Not a bug report:** Conor Greer, Marley Parker, Walker Mullinax, Sutton Welch and Charlie Chiesa were each un-tagged deliberately when two build-out bugs were fixed, and Ophelia's 2026 25m Fly was un-tagged by the third (the ageGroup-spacing fix). **Noah Hummel is the sole genuine first-time-ever tag in the live Week 5 data.** If any of these names come up as seemingly missing their tag, that is the correct, reviewed state — not an unresolved bug.

**Data-format stability:** The `hasAnyPriorQual` function has gone through **three rounds of data-format fixes** since it shipped and is now considered stable. The three bugs were: (1) field name — `league-results*.json` stores time as `r.time`, not `r.seconds`; (2) event scoping — original version only matched the same event, not any prior qualifying event; (3) ageGroup spacing — `swim-results.json` uses `"X & Under"` (spaced), standards table expects `"X&Under"` (no space). Each was caught, fixed, and covered by a regression test (Cases G, H/I/J, and K respectively). **Any future data source added to this function's scan set must have its schema conventions verified explicitly** against the standards-table key format (no-space `&Under`, `Boys`/`Girls` gender prefix, `YYYY-MM-DD` date, `seconds` field name) before being assumed compatible.
