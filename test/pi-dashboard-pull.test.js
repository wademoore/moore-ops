...
html = ('<!doctype html>' + 'x' * 1000000 + '<main class="today-panel upcoming-panel athletics-panel right-rail" data-sports-url="' + sports + '"><section class="now-next "></section><div class="centers-block"></div><footer class="sports-ticker"></footer></main>').encode()
def run(instant):
...
test('Pi validator rejects ordinary artifacts missing canonical NOW/NEXT or Centers markers', () => {
  const markerScript = String.raw`
import datetime, hashlib, importlib.util, json, pathlib
...
sports='https://example.lambda-url.us-east-2.on.aws/'
base='<!doctype html>'+'x'*1000000+'<main class="today-panel upcoming-panel athletics-panel right-rail" data-sports-url="'+sports+'"><section class="now-next "></section><div class="centers-block"></div><footer class="sports-ticker"></footer></main>'
def rejected(text):
  body=text.encode(); instant='2026-08-24T11:00:00.000Z'; artifact={'key':'dashboard-v2/releases/x/index.html','versionId':'v1','size':len(body),'sha256':hashlib.sha256(body).hexdigest()}; manifest={'schemaVersion':1,'artifactVersion':'dashboard-v2','generatedAt':instant,'artifact':artifact,'runtime':{'browserOrigin':'http://127.0.0.1:4173','sportsFeedUrl':sports}}
  try: m.validate(manifest,body,sports,now=datetime.datetime.fromisoformat('2026-08-24T11:05:00+00:00')); return False
  except ValueError: return True
print(json.dumps([rejected(base.replace('now-next ', 'old-events ')), rejected(base.replace('centers-block', 'school-strip'))]))
`;
  assert.deepEqual(JSON.parse(execFileSync('python', ['-c', markerScript], { cwd: process.cwd(), encoding: 'utf8' })), [true, true]);
});
