...
const asset = name => {
  if (name === 'first-day-title.png' || name === 'paper-lined.png') name = 'composition-reference.png';
...
const eventTime = event => event?.raw?.start?.dateTime ? new Date(event.raw.start.dateTime).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:'America/New_York'}) : 'All day';
...
function milestones(data) {
  ...
  return [...todayEvents(data),...(data.upcomingEvents||[])].filter(event=>eventKey(event)===key&&/\bfirst day(?: of school)?\b|school.*\bfirst day\b/i.test(`${event.title||''} ${event.subtitle||''}`));
}

function cutoff(data, hits) {
  if(data.firstDayLevel3Until)return new Date(data.firstDayLevel3Until);
  const times=[...todayEvents(data),...(data.upcomingEvents||[])].filter(event=>eventKey(event)===dateKey(data.today)&&/school|stonehouse|drop[ -]?off|depart|arrival|arrive|first day/i.test(`${event.title||''} ${event.subtitle||''}`)).map(event=>event?.raw?.end?.dateTime||event?.raw?.start?.dateTime).filter(Boolean).map(value=>new Date(value)).filter(value=>!Number.isNaN(value.getTime()));
  if(times.length)return new Date(Math.max(...times.map(value=>value.getTime()))+30*60*1000);
  return new Date(`${eventKey(hits[0])}T09:00:00-04:00`);
}
...
function shouldRenderFirstDayLevel3(data) {
  ...
  if(data.firstDayLevel3===true&&!hits.length)return true;
  return new Date(data.now||Date.now())<cutoff(data,hits);
}

function schedule(data) {
  const events=todayEvents(data).sort((a,b)=>eventStamp(a)-eventStamp(b)),now=new Date(data.now||Date.now()).getTime(),future=events.findIndex(event=>eventStamp(event)>=now);
  if(!events.length)return {now:null,next:null};
  if(future<0)return {now:null,next:null};
  return {now:events[future],next:events[future+1]||null};
}
...
function card(label,event,tone) {
  if(!event)return `<section class="fd-card fd-${tone}"><h2>${label}</h2><div class="fd-muted">Nothing else scheduled.</div></section>`;
  const owner=person(event),ownerLabel=owner==='myles'?'Myles':owner==='ophelia'?'Ophelia':owner==='both'?'Myles + Ophelia':'';
  return `${label==='NOW'?`<style>${COMPOSITION_CSS}</style>`:''}<section class="fd-card fd-${tone} person-${owner}"><h2>${label}</h2><div class="fd-event"><time>${esc(eventTime(event))}</time><div><strong>${esc(clean(event.title))}</strong>${event.subtitle?`<span>${esc(clean(event.subtitle))}</span>`:''}</div></div>${ownerLabel?`<div class="fd-owner">${ownerLabel}</div>`:''}</section>`;
}
...
function renderFirstDayLevel3(data) {
  ...
  const vars=[`--composition:url('${asset('composition-reference.png')}')`,`--paper:url('${asset('paper-lined.png')}')`,`--red:url('${asset('frame-red.png')}')`,`--purple:url('${asset('frame-purple.png')}')`,`--blue:url('${asset('frame-blue.png')}')`,`--brush-red:url('${asset('brush-red.png')}')`,`--brush-purple:url('${asset('brush-purple.png')}')`].join(';');
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>Moore Family Dashboard — First Day of School</title><style>${CSS}</style></head><body><main class="first-day-dashboard" style="${vars}"><img class="fd-art fd-title" src="${asset('first-day-title.png')}" alt="First Day of School!"><img class="fd-art fd-myles" src="${asset('myles-first-day.png')}" alt="Myles"><img class="fd-art fd-ophelia" src="${asset('ophelia-first-day.png')}" alt="Ophelia"><img class="fd-art fd-school" src="${asset('stonehouse-sea-stars.png')}" alt="Stonehouse Elementary Sea Stars"><div class="fd-clock"><time id="live-clock">${esc(now.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:'America/New_York'}))}</time><span id="live-date">${esc(now.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',timeZone:'America/New_York'}))}</span></div><div class="fd-grid">${card('NOW',s.now,'now')}${card('NEXT',s.next,'next')}<section class="fd-card fd-utility fd-weather"><h2>WEATHER</h2>${weatherHtml}</section><section class="fd-card fd-utility fd-dinner"><h2>DINNER</h2><div class="fd-dinner-copy"><strong>${esc(dinner.title||'Not set')}</strong>${dinner.subtitle?`<span>${esc(dinner.subtitle)}</span>`:''}</div></section><section class="fd-card fd-utility fd-coming"><h2>COMING UP</h2><div class="fd-coming-list">${coming.map(event=>`<div class="fd-coming-item"><time>${esc(new Date(`${eventKey(event)}T12:00:00`).toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'}))}</time><div><strong>${esc(clean(event.title))}</strong><span>${esc(eventTime(event))}${event.subtitle?` · ${esc(clean(event.subtitle))}`:''}</span></div></div>`).join('')||'<div class="fd-muted">Nothing coming up.</div>'}</div></section></div></main><script>(()=>{const z='America/New_York',c=document.getElementById('live-clock'),d=document.getElementById('live-date'),tick=()=>{const n=new Date();c.textContent=n.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:z});d.textContent=n.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',timeZone:z})};setInterval(tick,30000)})()</script></body></html>`;
}

export { renderFirstDayLevel3, shouldRenderFirstDayLevel3, schedule };
