...
function activityVisual(event, className) {
  ...
  if (url) return `<span class="${className} semantic-icon activity-visual has-logo category-${category}${event?.flagFootball ? ' flag-activity-visual' : ''}" aria-label="${esc(category)}">${fallback}<img src="${esc(url)}" alt="" onerror="this.parentElement.classList.remove('has-logo');this.remove()"></span>`;
...
function spotlightMark(url) {
  ...
  return `<span class="${base} activity-visual has-logo" aria-label="sports">${fallback}<img src="${esc(url)}" alt="" onerror="this.parentElement.classList.remove('has-logo');this.remove()"></span>`;
}
...
// Display-name collisions are labels; numeric team ids identify the rows.
function flagStandingLabels(rows = []) {
...
.today-panel{grid-column:1;grid-row:2;padding:16px 20px;display:flex;flex-direction:column;min-height:0}.upcoming-panel{grid-column:2;grid-row:2;align-self:start;height:calc(58% - 36px);padding:16px 20px}.athletics-panel{grid-column:2;grid-row:2;align-self:end;height:calc(40% + 38px);padding:16px 20px}.sports-ticker{grid-column:1/3;grid-row:3}.right-rail{grid-column:3;grid-row:1/4;display:grid;grid-template-rows:118px 172px minmax(0,1fr) 196px;gap:13px;min-height:0}
...
/* Retiring the 92px strip plus its 13px gap gives 105px to the main row.
   Against the old percentages, these offsets give Athletics 80px and Coming Up
   about 25px in either card layout, while preserving the inter-panel gap. */
...
/* A masthead and latest-result line need 32px more in the stacked multi-card layout. */
.dashboard.has-masthead.athletics-multi .upcoming-panel{height:calc(58% - 68px)}.dashboard.has-masthead.athletics-multi .athletics-panel{height:calc(40% + 70px)}
.dashboard.athletics-one .upcoming-panel{height:calc(72% - 50.6px)}.dashboard.athletics-one .athletics-panel{height:calc(26% + 52.7px)}
...
/* The wall runs Chromium 88: use explicit state, not the unsupported :has() selector. */
.activity-visual.has-logo>svg{visibility:hidden}
.activity-visual.has-logo{background:transparent}
...
.paper-panel,.rail-card{background:var(--surface-panel);border-color:rgba(130,92,32,.48);box-shadow:0 4px 12px rgba(45,29,11,.08),inset 0 0 18px rgba(90,65,28,.035)}
...
.dashboard[data-holiday-state="active"] .paper-panel,.dashboard[data-holiday-state="active"] .rail-card{border-color:var(--holiday-border-active)}
