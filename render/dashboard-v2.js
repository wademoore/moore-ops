...
function browserScript() {
  ...
  (() => {
    ...
    if(releaseManifestUrl&&currentGeneration){const checkRelease=async()=>{try{const response=await fetch(releaseManifestUrl,{cache:'no-store'});if(!response.ok)return;const manifest=await response.json();if(manifest?.schemaVersion===1&&manifest?.artifactVersion==='dashboard-v2'&&manifest.generatedAt&&manifest.generatedAt!==currentGeneration){if(location.pathname.endsWith('/level2.html'))location.replace('index.html');else location.reload()}}catch{}};setInterval(checkRelease,300000)}
...
.athletic-ribbon{height:46px;padding:3px 12px 3px 48px;gap:10px;font-size:21px;letter-spacing:.025em;color:#fff;text-shadow:0 1px 1px rgba(0,0,0,.25)}.athletic-ribbon span{line-height:1.2;padding:2px 0}.athletic-logo{width:38px;height:38px;padding:0;background:transparent;border-radius:0}.athletics-grid{height:calc(100% - 52px)}
